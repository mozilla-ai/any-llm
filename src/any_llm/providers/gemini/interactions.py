...
def _convert_usage(usage: Usage | None) -> ResponseUsage | None:
    ...
    # Google reports visible output and thoughts separately; OpenAI output includes both.
    # https://ai.google.dev/gemini-api/docs/interactions/streaming
    # https://github.com/openai/openai-python/blob/v2.18.0/src/openai/types/responses/response_usage.py
    output_tokens = (usage.total_output_tokens or 0) + reasoning_tokens
    return ResponseUsage(
        input_tokens=input_tokens,
        # OpenAI 3.7 requires cache_write_tokens, while 2.18 has no typed constructor field.
        input_tokens_details=InputTokensDetails.model_validate(
            {"cached_tokens": usage.total_cached_tokens or 0, "cache_write_tokens": 0}
        ),
...
def _messages_from_steps(
    ...
    for step in steps or []:
        if not isinstance(step, ModelOutputStep):
            # Default thinking can emit signature-only steps; this one-shot text path omits their metadata.
            # https://ai.google.dev/gemini-api/docs/interactions/streaming
            continue
...
def convert_interaction_to_response(
