...
The `azureopenai` provider uses one OpenAI SDK client at `/openai/v1/`. Set
`AZURE_OPENAI_ENDPOINT` and `AZURE_OPENAI_API_KEY`, or configure them explicitly:
...
### Migrating from the dated Azure API
...
- Replace `api_version="2025-03-01-preview"` (or another dated version) with
  ...
  raises a migration error; there is no dated-API fallback.
...
capability does not mean your resource has that deployment. The Azure integration
suite uses `AZURE_OPENAI_EMBEDDING_DEPLOYMENT`, `AZURE_OPENAI_IMAGE_DEPLOYMENT`,
`AZURE_OPENAI_TRANSCRIPTION_DEPLOYMENT`, and `AZURE_OPENAI_SPEECH_DEPLOYMENT` for
these operations; missing deployments are reported as verification skips,
not evidence that the API is unsupported. API-key tests do not verify live Entra
access. Files support is a separate change.
