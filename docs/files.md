...
| Gemini (Developer API) | Yes | Yes | Yes | Generated files | Yes |
...
not Vertex AI. Uploads, listing, metadata, generated-file downloads, and deletion
use `google-genai`'s `client.aio.files`. File IDs are Gemini resource names such as
`files/abc-123`; passing the bare suffix `abc-123` is accepted and canonicalized.
...
try:
    ...
    response = provider.completion(
        ...
        messages=[{
            "role": "user",
            "content": [
                {"type": "file", "file": {"file_data": uploaded.uri}},
                {"type": "text", "text": "Summarize this report."},
            ],
        }],
    )
...
the file URI in a model request. `FAILED` includes provider error details in
`error`. Native fields such as `uri`, `download_uri`, `source`, `sha256_hash`,
and `error` are preserved on `FileMetadata` extras.
...
Gemini marks user uploads as non-downloadable (`source="UPLOADED"`, no
`download_uri`). Download files generated by Gemini (for example video
generation) after confirming `downloadable` is true. The adapter retrieves
metadata first and rejects uploads with `InvalidRequestError` before opening the
content stream.

```python
metadata = provider.retrieve_file(file_id)
if metadata.downloadable:
    with provider.download_file(file_id) as chunks:
        with open("result.bin", "wb") as destination:
            for chunk in chunks:
                destination.write(chunk)
```

google-genai's `files.download()` either buffers the full body or writes it to a
destination. Gemini downloads in any-llm stream the same
`files/{id}:download?alt=media` response the SDK uses, exposing `status_code`
and headers on context entry and yielding one `chunk_size` body chunk at a time.
Exiting the context closes the upstream response.
...
request, so `max_retries=0` is one attempt). Uploads default to zero automatic
retries. `extra_headers` is merged into retrieve, list, delete, and download
requests. google-genai's resumable upload rebuilds session headers, so
