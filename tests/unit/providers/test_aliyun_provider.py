...
from any_llm.providers.aliyun.aliyun import AliyunProvider
from any_llm.types.completion import CompletionParams


def test_aliyun_provider_initialization() -> None:
    """Test that AliyunProvider can be instantiated with an API key."""
...
def test_aliyun_convert_completion_params_passthrough() -> None:
    """Test that _convert_completion_params passes through without modification."""
    params = CompletionParams(
        model_id="aliyun-model",
        messages=[{"role": "user", "content": "Hello"}],
        max_tokens=100,
    )
    result = AliyunProvider._convert_completion_params(params)
