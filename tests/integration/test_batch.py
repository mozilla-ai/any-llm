...
from any_llm.exceptions import BatchNotCompleteError, MissingApiKeyError
...
async def test_create_and_retrieve_batch(
    ...
    try:
        ...
        try:
            ...
            assert batch.input_file_id is not None  # May be empty string for Anthropic
...
@pytest.mark.asyncio
async def test_retrieve_batch_results_not_complete(
    provider: LLMProvider,
    provider_model_map: dict[LLMProvider, str],
    provider_client_config: dict[LLMProvider, dict[str, Any]],
) -> None:
    """Test that retrieving results of a non-completed batch raises BatchNotCompleteError."""
    try:
        llm = AnyLLM.create(provider, **provider_client_config.get(provider, {}))
        if not llm.SUPPORTS_BATCH:
            pytest.skip(f"{provider.value} does not support batch completions, skipping")
        ...
        try:
            ...
            with pytest.raises(BatchNotCompleteError) as exc_info:
                await llm.aretrieve_batch_results(batch.id)

            assert exc_info.value.batch_id == batch.id

            with contextlib.suppress(Exception):
                await llm.acancel_batch(batch.id)
...
@pytest.mark.asyncio
async def test_retrieve_batch_results_with_api_function(
    provider: LLMProvider,
    provider_model_map: dict[LLMProvider, str],
    provider_client_config: dict[LLMProvider, dict[str, Any]],
) -> None:
    """Test that aretrieve_batch_results API function raises BatchNotCompleteError for a fresh batch."""
    try:
        llm = AnyLLM.create(provider, **provider_client_config.get(provider, {}))
        if not llm.SUPPORTS_BATCH:
            pytest.skip(f"{provider.value} does not support batch completions, skipping")
