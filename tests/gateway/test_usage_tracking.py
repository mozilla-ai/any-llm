...
async def test_completion_accuracy(
    ...
    async with session_local() as db:
        user = (await db.execute(select(User).where(User.user_id == test_user["user_id"]))).scalar_one_or_none()
    ...
    async with session_local() as db:
        # Check usage log
        usage_logs = (
            (await db.execute(select(UsageLog).where(UsageLog.api_key_id == api_key_obj["id"]))).scalars().all()
        )
